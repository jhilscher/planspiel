...
import org.xml.sax.SAXParseException;
	...
	* @author Felix
 	*/
public class XMLRoleplayReader {
	private static Logger log = Logger.getLogger(XMLRoleplayReader.class);
	/**
	 * Variable of the domain-class type ReasonGlossary.
	 */
	...
	public String setXML() {
		...
		readXML();
		List<String> roleplays = readXML();
		DbService db;
	
		db = new DbService();
		for (String a : roleplays) {
			roleplay = new Roleplay(a);
			db.createNewSth(roleplay);
		}
		return "#";
	}
	...
	public List<String> readXML() {
		...
		try {
			...
			Document doc = docBuilder.parse(new File(rootPath + "/Roleplays.xml"));
	
			//normalizes the document
			doc.getDocumentElement().normalize();
	
			//Builds a NodeList of all sets
			NodeList listOfSets = doc.getElementsByTagName("Roleplay");
			...
			for (int i = 0; i < listOfSets.getLength(); i++) {
				...
				if (firstSettingNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					log.info("Roleplay: " + textFNList.item(0).getNodeValue().trim());
					//add the Object to the list
					roles.add(textFNList.item(0).getNodeValue().trim());
				}			
			}
			return roles; } catch (SAXParseException err) {
