...
import org.xml.sax.SAXParseException;
	 ...
	 * @author Felix
	 */
public class XMLActionsReader {
	private static Logger log = Logger.getLogger(XMLActionsReader.class);
	/**
	 * Variable of the domain-class type ActionGlossary.
	 */
...
public String setXML() {
	...
	readXML();
	List<String> actions = readXML();
	DbService db;

	db = new DbService();
	for (String a : actions) {
		action = new ActionGlossary(a);
		db.createNewSth(action);
	}
return "#";
}
...
public List<String> readXML() {
	...
	try {
		...
		Document doc = docBuilder.parse(new File(rootPath + "/Actions.xml"));

		//normalizes the document
		doc.getDocumentElement().normalize();

		//Builds a NodeList of all sets
		NodeList listOfSets = doc.getElementsByTagName("Action");
		...
		for (int i = 0; i < listOfSets.getLength(); i++) {
			...
			if (firstSettingNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				log.info("Aktion: " + textFNList.item(0).getNodeValue().trim());
