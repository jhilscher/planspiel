...
import org.xml.sax.SAXParseException;
	 ...
	 * @author Felix
	 */
public class XMLReasonsReader {
	private static Logger log = Logger.getLogger(XMLReasonsReader.class);
	/**
...
public String setXML() {
	...
	readXML();
	List<String> reasons = readXML();
	DbService db;

	db = new DbService();
	for (String a : reasons) {
		reason = new ReasonGlossary(a);
		db.createNewSth(reason);
	}
return "#";
}
...
public List<String> readXML() {
	...
	try {
		...
		Document doc = docBuilder.parse(new File(rootPath + "/Reasons.xml"));

		//normalizes the document
		doc.getDocumentElement().normalize();

		//Builds a NodeList of all sets
		NodeList listOfSets = doc.getElementsByTagName("Reason");
